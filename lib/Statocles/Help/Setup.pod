...
string C<blog>, which will get magically coerced into a L<file store
object|Statocles::Store::File>.
...
root directory of our repository, C<.>. Also like the blog, we need a theme to
allow for layout templates (we'll use the default theme again). Finally, we'll
need a URL root. Since we're using the root directory for our documents, we'll
use the root URL for our destination C</>.
...
everything in the C</static> directory, and give a URL root to match. Since
there is no processing, we don't need a theme here.
...
Create a file named C<about.yml> with the following content:
...
Then, run C<statocles build>. The C<about.yml> turns into C<about.html> in our C<build/>
directory.
