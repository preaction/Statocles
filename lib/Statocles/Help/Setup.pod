...
places, a build directory (a staging area), and a deploy object (the final
destination for the site).

For our site, we will make a C<build> directory for our staging area, where we
can verify that our site looks correct before we deploy.

Make sure this directory exists:

    $ mkdir build

For our destination, we need to build a L<deploy object|Statocles::Deploy> for
Git repositories using L<Statocles::Deploy::Git|Statocles::Deploy::Git>. Our
deploy object will copy files from the C<build> directory into the Git
repository. Our deploy will happen in the root directory of our site (C<.>).
This is good for a GitHub Pages site.
...
Just like the blog store, the C<build_store> will magically create a
Statocles::Store::File object from the path string.
